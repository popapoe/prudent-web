...
class Model {
	...
	async update() {
		...
		for(let task of this.repository.data.completed.generate_min_not_definitely_in()) {
			await this.repository.insert_completion_front(this.repository.data.completion_front.length, task);
		}
		for(let task of this.repository.data.completed.generate_max_not_definitely_out()) {
			await this.repository.insert_uncompletion_front(this.repository.data.uncompletion_front.length, task);
		}
	}
	// Take tasks according to the synchronization protocol, reading lines through the asynchronous function `read` and writing lines through `write`. Mutates this model.
	async take(read, write) {
		...
		for(let task of this.repository.data.completed.generate_max_definitely_in()) {
				...
				if(union_not_definitely_out.contains(task)) {
					...
					while(await read() !== "stop") {
						...
						if(this.repository.data.operations.has(key)) {
							...
							await this.repository.add_operation(new Operation(key, task, is_in, causes));
						}
					}
	...
	// Give tasks according to the synchronization protocol, reading lines through the asynchronous function `read` and writing lines through `write`.
	async give(read, write) {
		...
		while(await read() !== "stop") {
			...
			let task = this.repository.data.tasks.get(key);
			for(let operation of this.repository.data.completed.get_history(task).get_operations()) {
				write("operation");
				write(operation.key);
